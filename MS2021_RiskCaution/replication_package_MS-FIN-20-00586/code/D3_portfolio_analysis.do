...
cd "`work'/code/"
capture log close
log using ./log/D3_portfolio_analysis.log, replace
...
use "`work'/data/portfolio_debrief.dta", clear
* br budgetmethod id
...
esttab r1 r2 using "`work'/output/tables/debrief_reg", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) noconstant nogaps label rtf replace ///
       interaction(" x ") nobaselevels noomitted ///
       scalar(N_clust) ///
       eqlabels(none) ///
       mlabels("Var. maximizing" "Var. maximizing")
...
use "`work'/data/portfolio_choice_arm0.dta", clear
...
graph export "`work'/output/graphs/Figure3_portfolio_choice_variance_budget_binscatter.png", replace as(png) width(2100) height(1500)
graph export "`work'/output/graphs/Figure3_portfolio_choice_variance_budget_binscatter.pdf", replace as(pdf)
