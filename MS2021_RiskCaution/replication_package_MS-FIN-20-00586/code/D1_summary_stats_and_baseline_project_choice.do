...
cd "`work'/code/"
capture log close
log using ./log/D1_summary_stats_and_baseline_project_choice.log, replace
...
gmnlbeta attr_mean_n attr_variance_n if question==1 & base_sample==1 & arm==0, saving("`work'/data/created/gmnl_betas_set1_arm0.dta") replace nrep(`n_reps')
...
esttab set1arm0 set1arm0_cl using "`work'/output/tables/Table2_baseline_gmnl_cl", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
...
esttab set1arm0 set1arm0_cl set1arm1 set1arm1_cl using "`work'/output/tables/baseline_arm1_gmnl_cl", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) nogaps label pr2 rtf replace ///
       interaction(" x ") nobaselevels noomitted scalar(N_clust) ///
       eqlabels(none) ///
...
esttab set1arm0_ml set1arm1_ml using "`work'/output/tables/baseline_arm1_ml", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) nogaps label pr2 rtf replace ///
       interaction(" x ") nobaselevels noomitted scalar(N_clust) ///
       eqlabels(none) ///
...
esttab set1arm0_all set1arm0_cl_all using "`work'/output/tables/baseline_gmnl_cl_all_subjects", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) nogaps label pr2 rtf replace ///
       interaction(" x ") nobaselevels noomitted scalar(N_clust) ///
       eqlabels(none) ///
...
esttab set2arm0 set12arm0 set2arm0_cl set12arm0_cl using "`work'/output/tables/table4_set1_2_nest_gmnl_cl", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) nogaps label pr2 rtf replace ///
       interaction(" x ") nobaselevels noomitted scalar(N_clust) ///
       eqlabels(none) ///
       mlabels("Project choice\line GMNL" "Project choice\line GMNL" "Project choice\line Cond. logit" "Project choice\line Cond. logit")
* Baseline results table: Phase 1, Choice Scenario set 1
esttab set1arm0 set2arm0 set1arm0_cl set2arm0_cl using "`work'/output/tables/set1_2_gmnl_cl", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) nogaps label pr2 rtf replace ///
       interaction(" x ") nobaselevels noomitted scalar(N_clust) ///
       eqlabels(none) ///
...
esttab set2arm0 set2arm0_cl set2arm1 set2arm1_cl using "`work'/output/tables/baseline_set2_arm1_gmnl_cl", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) nogaps label pr2 rtf replace ///
       interaction(" x ") nobaselevels noomitted scalar(N_clust) ///
       eqlabels(none) ///
...
twoway ///
       ...
       scale(1.1)
graph export "`work'/output/graphs/choice_by_variance_pred1.pdf", replace as(pdf)
graph export "`work'/output/graphs/choice_by_variance_pred1.png", replace as(png) width(1800) height(1`n_reps')

* Distribution of preference parameters
* Appendix Figure 1
use "`work'/data/created/gmnl_betas_set1_arm0.dta", clear
...
gmnlbeta attr_mean_n attr_variance_n , saving("`work'/data/created/gmnl_betas.dta") replace nrep(`n_reps')
...
use "`work'/data/demographics_all.dta", clear
merge 1:1 id using "`work'/data/created/gmnl_betas.dta", nogen keep(1 3)
su elapsedtime
* Phase 1
summarize age numyearswork workrad_ind coef_rra discount_rate collegemath decisionscience if arm == 0 & base_sample == 1
* Phase 2
tabstat age numyearswork workrad_ind coef_rra discount_rate loss_averse collegemath decisionscience if base_sample == 1 & arm != 0, statistics(mean) by(arm)
tabstat age numyearswork workrad_ind coef_rra discount_rate loss_averse collegemath decisionscience if base_sample == 1 & arm != 0, statistics(sd) by(arm)
tabstat age numyearswork workrad_ind coef_rra discount_rate loss_averse collegemath decisionscience if base_sample == 1 & arm != 0, statistics(count) by(arm)
...
esttab * using "`work'/output/tables/balance_project_choice_set", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
...
use "`work'/data/demographics_all.dta", clear
merge 1:1 id using "`work'/data/portfolio_budget.dta", nogen keep(3)
...
esttab * using "`work'/output/tables/balance_portfolio_budget", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) noconstant nogaps label  replace ///
       scalar(F pval) ///
       eqlabels(none) rtf 
...
esttab * using "`work'/output/tables/balance_phase_2_arm", ///
       star(* 0.10 ** 0.05 *** 0.01) ///
       se(a2) b(a2) noconstant nogaps label  replace ///
       scalar(F pval) ///
       eqlabels(none) rtf 
