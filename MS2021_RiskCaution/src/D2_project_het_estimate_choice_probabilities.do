...
save "project_predicted_choice_het_justvar_20211020.dta", replace
* Table 4
...
esttab estgmnl_coef_rra estgmnl_unspent_ind estgmnl_workrad_ind estgmnl_discount_rate ///
       ...
       using "Table3_project_choice_heterogeneity_comp.csv", b(2) se(2) pr2(3) star(* 0.10 ** 0.05 *** 0.01) nocons nogaps nonotes replace
...
//        using "project_choice_heterogeneity_clogit.csv", b(2) se(2) pr2(3) star(* 0.10 ** 0.05 *** 0.01) nocons nogaps nonotes replace
* Plot
...
use "project_predicted_choice_het_justvar_20211020.dta", clear
merge 1:1 id question set question_group choice_set choice_option using "$data/project_predicted_choice_main.dta"
keep if _merge == 3
drop _merge
...
coefplot (mar_base \mar_collegemath \mar_decisionscience \mar_discount_rate \mar_spend \mar_risk \mar_prob_med_prize \mar_workrad_ind \mar_finance , ciopts(lcolor(black)) color(black)), keep(vv_*)  byopts(row(1)) graphregion(margin(l=50)) coeflabels(, notick labgap(-125)) xline(`base_line', lcolor(blue) lpattern(dash)) xline(0, lcolor(black)) headings(vv_base = "{bf:Baseline}" vv_averse = "{bf:Risk Aversion}" vv_unspent = "{bf:Budget Spending}" vv_norad = "{bf:R&D Experience}" vv_lowd = "{bf:Discount Rate}" vv_lowcollegemath = "{bf:College Math}" vv_lowdecisionscience = "{bf:Decision Science Classes}" vv_finance = "{bf:Finance or MBA}" vv_high_prob_med_prize = "{bf:Incentive Strength}" , labgap(-130)) yscale(noline alt) legend(off) scale(1.1) levels(95)

graph export "Figure2_project_choice_1_het.pdf", replace as(pdf)
graph export "Figure2_project_choice_1_het.png", replace as(png)
...
// esttab gmnl_all_het  ///
//        using "project_choice_heterogeneity_all", ///
//        star(* 0.10 ** 0.05 *** 0.01) ///
...
// graph export "project_choice_1_het_all.pdf", replace as(pdf)
// graph export "project_choice_1_het_all.png", replace as(png)
