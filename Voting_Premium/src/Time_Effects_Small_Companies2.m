%% Time effects for small companies
tic
clear
clc
...
PortfolioWeights = 1; % Use 'me' for value weighted and '1' for equally wegithed
...
SignalBefore = lagmatrix(SignalVote,-1) + lagmatrix(SignalVote,-2) + lagmatrix(SignalVote,-3) + lagmatrix(SignalVote,-4) ; % The signal in the base period is set up for investments in t+1
...
indVOT = aprts(SignalBefore, NP); % assigns ret break points that form TEN portfolios using NYSE BREAKS (the optional third argument)
...
indVOT = aprts(SignalAfter, NP); % assigns ret break points that form TEN portfolios using NYSE BREAKS (the optional third argument)
[resultsvwVOT,pretVOT] = tsregs(ret,indVOT,dates,PortfolioWeights,4,0);          % basic value-weighted 
% Do the proper regressions
...
%% Period Effects
tic
Signal = lagmatrix(SignalVote,0); % Itterate the lag to get the period you want (0 is the month after the vote)
Signal(Signal == 0) = nan;
